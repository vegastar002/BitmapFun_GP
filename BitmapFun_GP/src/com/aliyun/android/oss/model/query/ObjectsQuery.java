...
public class ObjectsQuery extends OSSQuery<OSSObjectSummary> {
    ...
    public void getTotalItems(PageMarker curMarker) throws OSSException {
    	int totalPage = 0;
        ...
        while (obj.getNextMarker() != null) {
        	totalPage = totalPage + obj.getItems().size();
        	Log.i("", "totalPage : "+totalPage);
        	curMarker.setNext(new PageMarker(obj.getNextMarker()));
        	curMarker.getNext().setPrevious(curMarker);
//        	getTotalItems(new PageMarker(obj.getNextMarker()));
        	getBucketTask.setMarker(curMarker.getContent());
